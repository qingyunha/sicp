  ...
  (let ([local-table (mlist '*table)])
    (define (lookup key)
      (define (iter table)
        (if (null? table)
            (display "not find")
            (if (eq? key (car (mcar table)))
                (cdr (mcar table))
                (iter (mcdr table)))))
      (iter (mcdr local-table)))
    (define (insert key value) 
      (set-mcdr! local-table (mcons (cons key value) (mcdr local-table))))
    (define (dispatch m)
      (cond ((eq? m 'lookup) lookup)
            ((eq? m 'insert) insert)
            (else (error "Unknow type" m))))
    dispatch))
...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ((variable? exp) (lookup-variable-value exp env))
        (else ((get (make exp)) exp env))))
  ;(applyy (eval (operator exp) env)
          ;(list-of-value (operands exp) env)))

(define (make exp) (car exp))
...
(define (set-variable-value! var val env)
  (define (env-loop env)
    (define (scan vars vals )
      (cond ((null? vars))
            (env-loop (enclosing-environment env))
            ((eq? var (mcar vars)) (set-mcar! val vals))
            (else (scan (mcdr vars) (mcdr vals)))))
    (if (eq? env empty-environment) 
        (error "unbound varible" var)
        (let ((frame (first-frame env)))
...
(define (eval-installer-package)
  (put 'set! eval-assignment)
  (put 'define eval-definition)
  (put 'if eval-if)
  (put 'lambda (lambda (exp env) (make-procedure ((lambda-parameters exp) exp)
                                                  (lambda-body exp)
                                                  env)))
  (put 'begin (lambda (exp env) (eval-sequence (begin-action exp) env)))
